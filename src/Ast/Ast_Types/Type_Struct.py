...
class Struct(Ast_Types.Type):
    ...
    __slots__ = ('struct_name', 'members', 'methods', 'member_indexs', 'size', 'rang')
    name = "STRUCT"
    pass_as_ptr = True
    no_load = False
    ...
    def __init__(self, name: str, members: list[KeyValuePair], module):
        ...
        for c, member in enumerate(members):
            self.members[member.key.var_name] = member.get_type()
            self.member_indexs.append(member.key.var_name)
        # print(self.members, members)
        ...
        # self.ir_type = ir.LiteralStructType([x for x in self.members.values()])
    ...
    def get_op_return(self, op, lhs, rhs):
        if op == "ind": #! Temporary !
            if rhs.name=="literal" and rhs.ret_type.name=="i32":
                return self.members[self.member_indexs[rhs.value]]
