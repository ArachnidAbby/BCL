...
from Ast.typing import Block, FunctionDef
...
class VariableAssign(ASTNode):
    ...
    def __init__(self, pos: SrcPosition, ptr: ExpressionNode,
                 value: ExpressionNode, block: Block, typ=Void()):
        self._position = pos
    ...
    def copy(self):
        out = VariableAssign(self._position, self.var_name.copy(),
                             self.value.copy(), block.Block.BLOCK_STACK[-1])
        out.explicit_typ = self.explicit_typ
        out.typ = self.typ
        return out

    def create_new_var(self, func: FunctionDef):
        ...
        if self.block.get_variable(name) is None:
        ...
        if variable is not None and variable.type.is_void():
            # if self.value.ret_type.name == 'ref':  #! TEMP
            #     self.block.get_variable(name).type = self.value.ret_type.typ
            # else:
            variable.type = self.value.ret_type.get_assign_type(func, self.value)
    ...
    def post_parse(self, func: FunctionDef):
    ...
    def resolve_lifetime_coupling(self, func: FunctionDef):
    ...
    def set_not_constant(self, func: FunctionDef):
    ...
    def eval_impl(self, func: FunctionDef):
        ...
        if typ.needs_dispose and self.is_declaration:
            block.get_current_block().register_dispose(func, ptr)
